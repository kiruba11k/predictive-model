...
async def learn_upload(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Resolve columns directly from dataframe headers to avoid runtime helper-scope issues
        header_probe_row = {column: '' for column in df.columns}
        pain_point_key = find_pain_point_key(header_probe_row)
        success_key = resolve_success_key(header_probe_row)
